...
async def matches():
    return await run_blocking(service.matches_payload)


@router.get('/api/predict')
async def predict(params: dict = Depends(query_params)):
    return await run_blocking(service.predict_payload, params)
...
async def clear_cache():
    return await run_blocking(service.football_clear_cache_payload)


@router.get('/api/football/prepare_ml_data')
async def prepare_ml_data():
    return await run_blocking(service.prepare_ml_history_data_payload)


@router.get('/api/football/diagnostics')
async def diagnostics(params: dict = Depends(query_params)):
    return await run_blocking(service.football_diagnostics_payload, params)


@router.get('/api/football/review')
async def review(params: dict = Depends(query_params)):
    return await run_blocking(service.football_review_payload, params)


@router.get('/api/football/professional-status')
async def professional_status():
    return await run_blocking(service.football_professional_status_payload)


@router.get('/api/calibrate')
async def calibrate(params: dict = Depends(query_params)):
    return await run_blocking(service.calibrate_payload, params)


@router.get('/api/calibrate/list')
async def calibrate_list():
    return await run_blocking(service.calibrate_list_payload)


@router.get('/api/calibrate/clear')
async def calibrate_clear():
    return await run_blocking(service.calibrate_clear_payload)


@router.get('/api/backtest')
async def backtest(params: dict = Depends(query_params)):
    return await run_blocking(service.backtest_payload, params)


@router.get('/api/backtest/threshold')
async def threshold():
    return await run_blocking(service.threshold_payload)


@router.get('/api/model/status')
async def model_status():
    return await run_blocking(service.model_status_payload)


@router.get('/api/model/backtest_stats')
async def backtest_stats(params: dict = Depends(query_params)):
    return await run_blocking(service.backtest_stats_payload, params)


@router.get('/api/predictions')
async def predictions():
    return await run_blocking(service.predictions_payload)


@router.get('/api/predictions/export')
async def predictions_export():
    return await run_blocking(service.predictions_export_payload)


@router.get('/api/sync/status')
async def sync_status():
    return await run_blocking(service.sync_status_payload)


@router.get('/api/sync/trigger')
async def sync_trigger():
    return await run_blocking(service.sync_trigger_payload)


@router.get('/api/sync/hide_failed')
async def sync_hide_failed():
    return await run_blocking(service.sync_hide_failed_payload)
