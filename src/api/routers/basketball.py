...
async def value_bets(date: Optional[str] = None, threshold: float = 0.05):
    """价值投注推荐。"""
    return await run_blocking(service.basketball_value_payload,
...
async def track(date: Optional[str] = None):
    """触发一次实时赔率轮询，累积盘路快照。"""
    return await run_blocking(service.basketball_track_payload, {'date': [date]})


@router.get('/movement')
async def movement(match_id: Optional[str] = Query(default=None)):
    """赔率走势汇总。"""
    return await run_blocking(service.basketball_movement_payload,
                              {'match_id': [match_id]})
