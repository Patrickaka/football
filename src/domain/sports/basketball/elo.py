...
与 football/elo.py 同架构。持久化通过 foundation/store 的三张表
（bb_elo_rating / bb_elo_history / bb_elo_recent_form），由 EloStore 封装。
"""
...
from .elo_store import EloStore

logger = logging.getLogger(__name__)

# ==================== ELO 配置 ====================

# 数据已迁至 foundation/store 的三张表，kv_store 中的同名 key 归档不再读写。
...
class BasketballELORatingSystem:
    ...
    def __init__(self, store=None):
        """store 可注入，便于测试用 SQLite 内存库；生产传 EloStore(db)。"""
    ...
    def _save(self):
        ...
        try:
            self._store.save(
                self.ratings, self.history, self.recent_form,
