...
# exp 的指数夹紧范围。**不夹会溢出**：θ 上限 12，而特征值在极端赔率下能到
# 上千，两者相乘轻易越过 `exp` 的定义域（float 在 709.78 以上就是 inf）。
#
# 取 20 而不是贴着上限：`exp(20)` 已经有 4.9e8 的比值，足以表达任何合理的
# 倾斜；再大只会让归一化时的浮点精度变差，换不来额外的表达力。
# **20 到 709 之间行为等价**——变异验证里把它改成 700 测试不会红，
# 那是等价变异不是漏测；改到 710 以上立刻红。
EXP_CLAMP = 20.0
