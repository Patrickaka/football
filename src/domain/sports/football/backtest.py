# -*- coding: utf-8 -*-
"""回测：把历史预测和实际赛果对齐，算命中、偏差与调参建议。

**没有存储**：所有入口收的都是记录列表。取历史（`*_from_history` 那一族）
和命令行留在适配层 `src/football/backtest.py`。

`_normalize_1x2_probs` 是本仓第四份**逐字相同**的 1X2 归一化
（两万条随机语料零差异，AST 去名后逐字相同），已合并到
`settlement.normalize_1x2_probs`（判据 11）。

时间层权重原来是「延迟 import 失败就退回 `1.0/0.5` 两档」——那个兜底
**从来走不到**（适配层一直导出得到），而真值是 0.35/0.55/0.75/0.9/1.0，
四个层全不一样。现在直接引领域实现，兜底删掉。
"""
...
from .policy import PARAM_RANGES, get_handicap_bucket, get_total_bucket
from .quality import filter_quality_records
from .settlement import normalize_1x2_probs as _normalize_1x2_probs
from .settlement import time_layer_weight
...
def apply_diagnostic_tuning(records: List[Dict],
                            ...
                            handicap: float = None,
                            dry_run: bool = True,
                            get_prediction_policy=None,
                            save_tuning_params=None) -> Dict:
    ...
    if get_prediction_policy is None:
        return {'applied': False, 'reason': 'policy_reader_not_injected'}
...
def optimize_prediction_parameters(records: List[Dict],
    ...
    save_result = None
    if save_best and best and save_tuning_params is not None:
        try:
