# -*- coding: utf-8 -*-
"""联赛级 SPF 策略的准入闸门：训练/留出两段各自达标才放行。

**没有存储**：读缓存的策略档案（`load_production_league_spf_policy`）
留在适配层 `src/football/production_league_gate.py`。

`wilson_interval` 从 `stats` 引——迁移前它住在 `professional_monitoring`，
而那个模块又反过来延迟 import 本模块的 `build_production_league_spf_policies`。
"""
