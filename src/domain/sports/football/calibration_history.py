...
**没有存储**：`estimate_history_calibration` 收的是记录列表，
读历史、读兜底档案、缓存 profile 都留在适配层
`src/football/history_calibration.py`。

`_quality_weight` 原来那条 `from .sample_quality import` 是延迟 import，
现在直接引领域层的 `quality`——同一个函数，只是不再绕适配层。
"""

import math
from typing import Dict, Iterable, List, Tuple

from .quality import assess_record_quality
