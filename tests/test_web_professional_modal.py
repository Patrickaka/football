...
class WebProfessionalModalTests(unittest.TestCase):
    ...
    def test_web_recommendations_use_accuracy_gates(self):
        ...
        self.assertIn("const FOOTBALL_ANALYSIS_CONCURRENCY = 1;", html)
        self.assertIn("const FOOTBALL_BATCH_SIZE = 6;", html)
        self.assertIn("await recoverTimedOutChunk(matches, indexes);", html)
    ...
    def test_football_list_uses_compact_joint_market_summary(self):
        ...
        self.assertIn('renderFootballMarginalReference(standardPrediction?.probs,', html)
        self.assertIn('<details class="football-marginal-reference">', html)
        self.assertNotIn("compactProbLine(standardPrediction.probs, ['胜','平','负'])", html)
