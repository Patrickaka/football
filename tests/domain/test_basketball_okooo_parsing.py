...
class RflistGoldenTests(unittest.TestCase):
    def test_all_shapes(self):
        for i, rflist in enumerate(RFLISTS):
            with self.subTest(rflist=rflist):
                self.assertEqual(as_json(new.parse_rflist(rflist)),
                                 GOLDEN[f'rflist:{i}'])
...
class LineTrendGoldenTests(unittest.TestCase):
    def test_all_shapes(self):
        for i, history in enumerate(HISTORIES):
            for kind in ('ah', 'ou', 'ml'):
                with self.subTest(n=len(history), kind=kind):
                    self.assertEqual(as_json(new.analyze_line_trend(history, kind)),
                                     GOLDEN[f'trend:{i}:{kind}'])


class ScheduleGoldenTests(unittest.TestCase):
    DATES = ['2026-08-26', '2026-08-27', '2026-09-01']

    def test_raw_rows(self):
        for date in self.DATES:
            with self.subTest(date=date):
                self.assertEqual(as_json(new.parse_schedule(HUNHE_HTML, date)),
                                 GOLDEN[f'schedule:{date}'])

    def test_full_fetch_filters_to_the_live_card(self):
        """抓取在解析之上多做一步：剔除完场、按开赛时刻撤下已开赛的。"""
        for date in self.DATES:
            with self.subTest(date=date):
                fetcher = new.OkoooScheduleFetcher(
                    transport=lambda url: HUNHE_HTML, now_fn=lambda: NOW)
                live = fetcher.fetch(date)
                raw = GOLDEN[f'schedule:{date}']
                self.assertLessEqual(len(live), len(raw))
...
class BookRowGoldenTests(unittest.TestCase):
    def test_book_rows(self):
        for kind, html in DETAIL_PAGES.items():
            with self.subTest(kind=kind):
                self.assertEqual(as_json(new.parse_book_rows(html, kind)),
                                 GOLDEN[f'books:{kind}'])

    def test_average_row(self):
        for kind, html in DETAIL_PAGES.items():
            with self.subTest(kind=kind):
                self.assertEqual(as_json(new.parse_average_row(html, kind)),
                                 GOLDEN[f'avg:{kind}'])
    ...
    def test_consensus(self):
        for kind, html in DETAIL_PAGES.items():
            with self.subTest(kind=kind):
                self.assertEqual(
                    as_json(new.consensus_from_books(
                        new.parse_book_rows(html, kind), kind)),
                    GOLDEN[f'consensus:{kind}'])

    def test_consensus_of_no_books(self):
        for kind in ('ml', 'ah', 'ou'):
            with self.subTest(kind=kind):
                self.assertEqual(as_json(new.consensus_from_books([], kind)),
                                 GOLDEN[f'consensus_empty:{kind}'])
...
class BundleGoldenTests(unittest.TestCase):
    def test_bundle(self):
        self.assertEqual(as_json(new.build_bundle(SAMPLE_MATCH_ID, DETAIL_PAGES)),
                         GOLDEN['bundle'])
