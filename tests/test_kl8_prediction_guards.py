...
class KL8PredictionGuardTests(unittest.TestCase):
    ...
    def test_reference_select_5_and_6_use_per_play_hot_window(self):
        for pick in (5, 6):
            strategy = resolve_play_strategy(f'select_{pick}')

            self.assertEqual(strategy['final_selection_mode'], 'top_ranked')
            ...
            expected_window = 75 if pick == 5 else 150
            self.assertEqual(strategy['baseline_type'], f'single_hot{expected_window}_weak_signal')
            self.assertEqual(strategy['window_size'], expected_window)
    ...
    def test_reference_plays_use_honest_coverage_selection(self):
        ...
        self.assertIn('single_hot75', select5['strategy_id'])
    ...
    def test_predict_all_includes_select_8_9_10_and_fushi_10_11(self):
        ...
        self.assertEqual(
            ...
            'select_5_single_hot75_weak_signal_v3',
        )
        ...
        for key in [f'select_{pick}' for pick in range(3, 11)]:
            expected_mode = 'top_ranked' if key in {'select_5', 'select_6'} else 'shape_balanced'
            self.assertEqual(
        ...
        self.assertEqual(result['select_6']['accuracy_profile']['selected_mode'], 'top_ranked')
