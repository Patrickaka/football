...
from src.webapp import caching, lazy_modules
...
class ServerSsqRouteTests(unittest.TestCase):
    ...
    # 所以 patch 要打在 `src.webapp.caching` 上。**打错地方不会报错**，
    # 只是什么也没替换掉，测试看着通过其实测的是真实调用。
    @patch('src.webapp.caching._serve_cached')
    ...
    @patch('src.webapp.lazy_modules.ssq_clear_cache')
    @patch('src.webapp.lazy_modules.ssq_run_prediction')
